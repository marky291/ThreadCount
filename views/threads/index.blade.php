   ...
   @if (count($threads) > 0)
       @foreach($threads as $thread)
           <div class="thread-item px-6 py-5 hover:bg-grey-lighter rounded flex justify-between items-center">
               ...
               <div class="flex-1">
                   <h4 class="font-bold md:font-semibold mb-3 tracking-tight text-black hover:text-black"><a href="/threads/show?slug={{$thread['slug']}}" class="text-grey-darkest">{{ ucwords($thread['title']) }}</a></h4>
                   <p class="text-grey-dark text-xs">
                       <span class="text-blue uppercase">
                           <a title="View all threads created by {{ ucwords($thread['username']) }}" href="/threads/user?username={{ $thread['username'] }}">{{ $thread['username']}}</a>
                       </span>
                       posted at {{ $thread['created_at'] }}</p>
               </div>
